...
if (isset($_GET['addform']))
{
  ...
  if ($_POST['password'] != '')
  {
    $password = md5($_POST['password'] . 'football');
...
if (isset($_POST['action']) and $_POST['action'] == 'Edit')
{
  ...
  try
  {
    $sql = 'SELECT id, name, email FROM info WHERE id = :id';
    $s = $pdo->prepare($sql);
...
if (isset($_POST['action']) and $_POST['action'] == 'Delete')
{
  ...
  try
  {
    $sql = 'SELECT id FROM players WHERE authorid = :id';
    $s = $pdo->prepare($sql);
    $s->bindValue(':id', $_POST['id']);
    $s->execute();
  }
  ...
  try
  {
    $sql = 'DELETE FROM players WHERE authorid = :id';
    $s = $pdo->prepare($sql);
    $s->bindValue(':id', $_POST['id']);
    $s->execute();
  }
